...
static void wafie_load_main_configs(EvaluationRequest *request) {
    ...
    for (size_t i = 0; i < 2; i++) {
        char conf_file[strlen(main_config_files[i]) + strlen(request->config_path) + 2];
        snprintf(conf_file, sizeof(conf_file), "%s/%s", request->config_path, main_config_files[i]);
        // fprintf(stdout, "loading rule file: %s\n", conf_file);
        int const ret = msc_rules_add_file(request->rules, conf_file, &error);
        if (ret < 0) {
...
static void wafie_load_modescurity_rules_configs(EvaluationRequest *request) {
    ...
    // 7 = strlen("/rules") + 1
    char rules_path[strlen(request->config_path) + 7];
    snprintf(rules_path, sizeof(rules_path), "%s/rules", request->config_path);
    struct dirent *entry;
    ...
    while ((entry = readdir(dp))) {
        if (entry->d_type == DT_REG) {
            ...
            int const ret = msc_rules_add_file(request->rules, rule_file, &error);
            if (ret < 0) {
                fprintf(stderr, "problems loading the rules --\n");
                fprintf(stderr, "%s\n", error);
                if (error != NULL) {
